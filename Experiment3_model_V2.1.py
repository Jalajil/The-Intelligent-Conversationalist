# In this version we are training the model without Daei's sentiment score and without 'Wants to Covert' experiment 3 data
...
# Paths to data folders (only Accepted Islam and Interested in Islam are used)
accepted_islam_path = r'D:\Roken Alhewar Datasset\filtered_results1\Accepted_Islam'
interested_in_islam_path = r'D:\Roken Alhewar Datasset\filtered_results1\Interested_in_Islam'

# Load data
data_accepted = load_data_from_folder(accepted_islam_path)
data_interested = load_data_from_folder(interested_in_islam_path)

# Combine and preprocess data (wants_to_convert data removed)
data = data_accepted + data_interested
...
def scheduler(epoch, lr):
    # After 10 epochs, multiply learning rate by 0.9 each epoch
    if epoch < 10:
        return lr
    else:
        new_lr = lr * 0.9
